            ...
            for (int iFile = 0; iFile < NB_FILES; iFile++)
            {
                ...
                if (pFileData != NULL)
                {
                    ...
                    fscanf_s(pFileData, "%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n",
                        &row.id,
                        &row.attitude.roll, &row.attitude.pitch, &row.attitude.yaw,
                    ...
                    // Write computed vectors to train/test sets
                    for (int iRow = 0; iRow < NB_VECTOR && !feof(pFileData); iRow++)
                    {
                        // Compute user acceleration as vector
                        vector = sqrt(row.userAcceleration.x * row.userAcceleration.x +
                                      row.userAcceleration.y * row.userAcceleration.y +
                                      row.userAcceleration.z * row.userAcceleration.z);
